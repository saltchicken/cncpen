...
def apply_image_mask(strokes: List[PenStroke], sampler: Any, threshold: float) -> List[PenStroke]:
    masked_strokes = []
    step_res = 1.0 
        ...
        for i in range(steps + 1):
            ...
            if sampler.get_darkness(pt.x, pt.y) > threshold:
                ...
                if len(current_segment) > 1:
                    masked_strokes.append(stroke.update_geometry(LineString(current_segment)))
                current_segment = []
...
def apply_roughening(strokes: List[PenStroke], amplitude: float, step: float) -> List[PenStroke]:
    if amplitude <= 0.0:
        return strokes
    return [
...
def apply_fisheye(strokes: List[PenStroke], factor: float, centroid: Any, max_r: float) -> List[PenStroke]:
    ...
    for stroke in strokes:
        ...
        distorted_strokes.append(stroke.update_geometry(LineString(warped_coords)))
        
    return distorted_strokes

def apply_clipping(strokes: List[PenStroke], boundary: Polygon) -> List[PenStroke]:
...
def apply_transform(strokes: List[PenStroke], angle: float, origin: Any, simplify_tol: float) -> List[PenStroke]:
    transformed_strokes = []
