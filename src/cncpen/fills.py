...
from .pipeline import PenStroke, PipelineHistory
from .filters import (
    apply_image_mask, 
    apply_roughening, 
    apply_fisheye, 
...
class ImageSampler:
    ...
    def get_darkness(self, x: float, y: float) -> float:
        ...
        px = max(0, min(int(((x - self.minx) / self.width) * (self.img.width - 1)), self.img.width - 1))
        py = max(0, min(int((1.0 - ((y - self.miny) / self.height)) * (self.img.height - 1)), self.img.height - 1))
...
def _ensure_geom(shape):
    poly = shape if isinstance(shape, BaseGeometry) else (Polygon() if len(shape) < 4 else Polygon(shape))
    return poly if poly.is_valid and poly.area > 0 else poly.buffer(0)


def generate_pipeline(filler, shape, angle=0.0, fisheye=0.0, image=None, simplify=0.0, **kwargs) -> List[PenStroke]:
    ...
    initial_strokes = [PenStroke(geometry=line) for line in all_raw_lines if not line.is_empty]
    ...
    if sampler and not getattr(filler, 'handles_image_natively', False):
        state_manager.apply_filter(partial(apply_image_mask, sampler=sampler, threshold=kwargs.get('threshold', 0.1)))

    # 2. Roughening
    if kwargs.get('roughen_amp', 0.0) > 0.0:
        state_manager.apply_filter(partial(apply_roughening, amplitude=kwargs.get('roughen_amp'), step=kwargs.get('roughen_step', 1.0)))

    # 3. Fisheye Distortion
    if fisheye != 0.0:
        state_manager.apply_filter(partial(apply_fisheye, factor=fisheye, centroid=centroid, max_r=max_r))

    # 4. Final Clip and Transform Cleanup
    for p in polygons:
        state_manager.apply_filter(partial(apply_clipping, boundary=p))
        state_manager.apply_filter(partial(apply_transform, angle=angle, origin=centroid, simplify_tol=simplify))
...
def load_plugins():
    ...
    for file_path in plugins_dir.rglob("*.py"):
        ...
        module_name = f"cncpen.plugins.{".".join(list(relative_path.parts[:-1]) + [relative_path.stem])}"
        try:
