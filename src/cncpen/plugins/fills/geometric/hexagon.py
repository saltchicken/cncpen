...
class HexagonFill:
    ...
    def generate(self, shape: BaseGeometry, radius: float = 3.0, **kwargs: Any) -> List[LineString]:
        ...
        # 1. Generate unique edges and map quantized coordinates to exact floats
        unique_edges = set()
        pts_map = {}
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                for i in range(6):
                    ...
                    vx = cx + radius * math.cos(angle_rad)
                    vy = cy + radius * math.sin(angle_rad)
