...
from .fills import generate_pipeline
from .fills import load_plugins
from .machine import PenConfig
from .machine import PenTool
from .utils import DXFReadError
    ...
    with PenTool(config, output_filename=args.output) as pen:
        ...
        if args.pattern and closed_polys:
            ...
            if fill_class:
                ...
                final_strokes = generate_pipeline(filler, combined_geom,
                                                  **vars(args))

                # Extract raw coordinates back for hardware shell processing
                raw_fill_coords = [
                    list(stroke.geometry.coords) for stroke in final_strokes
                ]
