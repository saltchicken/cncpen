...
def process_fills(closed_polys: List[Polygon],
                  paths_to_draw: List[List[Tuple[float,
                                                 float]]], global_config: dict,
    ...
    for step_def in fill_definitions:
        ...
        if "pattern" in step_def:
            ...
            if not fill_class:
                print(f"Warning: Unknown fill pattern '{pattern_name}'",
                      file=sys.stderr)
                continue
        ...
        # 2. PROCESS PATH MODIFICATIONS
            ...
            if not mod_class:
                print(f"Warning: Unknown modification '{mod_name}'",
                      file=sys.stderr)
                continue
            ...
            print(
                f"Warning: Step must contain 'pattern' or 'modification'. Ignored: {step_def}",
                file=sys.stderr)
...
def main() -> None:
    ...
    with PenTool(pen_config, output_filename=config['output']) as pen:
        ...
        # 3. Process Fills
        process_fills(closed_polys, paths_to_draw, config, fill_definitions,
                      pen)
